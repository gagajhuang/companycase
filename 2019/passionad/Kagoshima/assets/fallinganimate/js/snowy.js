...
(function()
{
	this.Snowy = (function()
	{
		...
		function Snowy(container, count, move)
		{
			...
			this.particleImage.src = './assets/fallinganimate/img/leaf2.png';
			//On window resize
			$(window).resize(function() { self.onResize(); }).trigger("resize");
			//Create camera 雪花數量
			this.camera = new THREE.PerspectiveCamera(75, this.width / this.height, 1, 500);
			//Set camera Z position
			this.camera.position.z = 1000;
			this.camera.opacity =  (Math.random() * 0.5) + 0.3;
			...
			for (var i = 0; i < count; i++)
			{
				...
				particle.position.y = Math.random() * 3500 - 1000;
			...
			//Start main loop
			setInterval(function() { self.loop(); }, 1000 / 20);/*速度調整*/
		}
