$(document).ready(function () {
  ...
  // resize 時將延遲秒數歸零
  const aa = document.querySelectorAll('.combination__content')
  function delayChange(){
    var wdth = $(window).width();
    if (wdth <= 769) {
      for (let i = 0; i < aa.length; i++) {
        if (aa[i].dataset.aosDelay) {
          aa[i].dataset.aosDelay = 0
        }
      }
    }else{
      for (let i = 0; i < aa.length; i++) {
        if (aa[i].dataset.aosDelay) {
          aa[1].dataset.aosDelay = 500
          aa[2].dataset.aosDelay = 1000
        }
      }
    }
  }
